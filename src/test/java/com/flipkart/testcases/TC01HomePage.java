...
public class TC01HomePage extends BaseClass {
	...
	public void searchProduct(String products) {
		...
		ExtentReportManager.test.info("Entered the Product in Textbox");
		...
		result.adjustSlider();
		ExtentReportManager.test.info("Slider are adjust properly");
		System.out.println("Slider are adjust properly");

		result.selectMaxPriceDrop("20000");
		ExtentReportManager.test.info("Max price is selected to 20,000");
		System.out.println("Max price is selected to 20,000");

		result.selectMinPriceDrop("10000");
		ExtentReportManager.test.info("Min price is selected to 10,000");
		...
		ExtentReportManager.test.pass(MarkupHelper.createLabel("All the Steps are Pass in Search Iphone Test", ExtentColor.GREEN));
	}
